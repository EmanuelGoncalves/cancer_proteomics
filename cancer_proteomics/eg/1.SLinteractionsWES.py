...
from crispy.DataImporter import Proteomics, GeneExpression, WES
...
if __name__ == "__main__":
    # Args
    #
    parser = argparse.ArgumentParser(description="Run LMM Protein/GExp ~ WES")
    ...
    # Data-sets
    #
    prot, gexp, wes = Proteomics(), GeneExpression(), WES()

    # Samples
    #
    samples = set.intersection(
        set(prot.get_data()), set(gexp.get_data()), set(wes.get_data())
    )
    ...
    if args.genes != "None":
        ...
        # Protein ~ CRISPR LMMs
        #
        prot_lmm = LMModels(y=prot.T[args.genes], x=wes.T, transform_x="None", institute=False).matrix_lmm()
        prot_lmm.to_csv(
            f"{RPATH}/lmm_protein_wes/{'_'.join(args.genes)}.csv.gz",
            index=False,
            compression="gzip",
        )
        ...
        #
        gexp_lmm = LMModels(y=gexp.T[args.genes], x=wes.T, transform_x="None", institute=False).matrix_lmm()
        gexp_lmm.to_csv(
            f"{RPATH}/lmm_gexp_wes/{'_'.join(args.genes)}.csv.gz",
            index=False,
            compression="gzip",
        )
        ...
        # Assemble protein
        #
        ddir = f"{RPATH}/lmm_protein_wes/"
        prot_table = pd.concat(
            [pd.read_csv(f"{ddir}/{f}") for f in os.listdir(ddir) if f.endswith(".csv.gz")],
            ignore_index=True,
            sort=False,
        ).sort_values("fdr")[LMModels.RES_ORDER]
        prot_table.to_csv(
            f"{RPATH}/lmm_protein_wes.csv.gz", index=False, compression="gzip"
        )

        # Assemble gexp
        #
        ddir = f"{RPATH}/lmm_gexp_wes/"
