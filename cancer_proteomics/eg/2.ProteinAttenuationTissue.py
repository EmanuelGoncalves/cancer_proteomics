...
y_cnv_amp = cnv_amp[cnv_amp.sum(1) >= cn_thres]

g_scores = []
for g in y_cnv_amp.index:
    g_res = dict(gene=g)

    for (dtype, df) in [("gexp", gexp), ("prot", prot)]:
        df_svm = (
            pd.concat(
                [df.loc[g].rename("x"), y_cnv_amp.loc[g].rename("y")], axis=1, sort=False
            )
            .loc[samples]
            .dropna()
        )

        if df_svm["y"].sum() < cn_thres:
            continue

        g_cv_scores = []
        g_cv = StratifiedShuffleSplit(test_size=0.2, n_splits=100)
        for i, (idx_train, idx_test) in enumerate(g_cv.split(df_svm[["x"]], df_svm["y"])):
            g_svm = LogisticRegressionCV(cv=3, scoring="balanced_accuracy").fit(
                df_svm[["x"]].iloc[idx_train], df_svm["y"].iloc[idx_train]
            )

            g_svm_pred = g_svm.predict(df_svm[["x"]].iloc[idx_test])

            if np.sum(g_svm_pred) == 0:
                continue

            g_svm_score = metrics.average_precision_score(df_svm["y"].iloc[idx_test], g_svm_pred)

            g_cv_scores.append(g_svm_score)

        g_res[dtype] = np.nanmean(g_cv_scores) if len(g_cv_scores) > 0 else np.nan

        LOG.info(f"Gene={g}; Dtype={dtype}; Score={g_res[dtype]:.2f}")

    g_scores.append(g_res)
g_scores = pd.DataFrame(g_scores).dropna()
g_scores = g_scores.assign(patt=g_scores.eval("gexp - prot")).sort_values("patt", ascending=False)
...
order = list(
    plot_df.groupby("context")["att_context"]
    .median()
...
_, ax = plt.subplots(
    1, 1, figsize=(4, 0.05 * len(set(plot_df["context"])) * len(order)), dpi=600
)
...
plot_df = (
    patt.query("len > 15")
    .groupby("context")[["att_context", "amp", "len"]]
    .agg({"att_context": np.median, "amp": np.sum, "len": np.median})
)
