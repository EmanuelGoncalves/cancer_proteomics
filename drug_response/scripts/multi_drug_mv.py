...
model_path = os.path.join(configs['work_dir'], 'models')
...
if configs['do_cv']:
    ...
    for train_index, test_index in cv.split(merged_df_train):
        ...
        train_dataset = ProteinDataset(train_df, train_ic50, mode='train', logger=logger)
        val_dataset = ProteinDataset(val_df, val_ic50, mode='val', logger=logger)
...
train_dataset = ProteinDataset(train_df, train_ic50, mode='train', logger=logger)
test_dataset = ProteinDataset(test_df, test_ic50, mode='test', logger=logger)
